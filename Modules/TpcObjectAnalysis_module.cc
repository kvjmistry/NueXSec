...
// Modified by K Mistry 09/09/2019
...
namespace xsecAna {class TpcObjectAnalysis;}
...
private:
    ...
    // These should be art::InputTag but several pandora functions want strings!
...
void xsecAna::TpcObjectAnalysis::analyze(art::Event const & e) {
    ...
    for (size_t tpc_counter = 0; tpc_counter < tpcobj_h->size(); tpc_counter++) {
        ...
        for (auto const pfp : pfps_from_tpcobj) {
            ...
            if(mcghost.size() == 0) {
                std::cout << "[Analyze] No matched MC Ghost to PFP!" << std::endl;
                
                // If Overlay then we assume unmatched cases are Cosmic
                if (_is_overlay) {
                    std::cout << "[Analyze] Unmatched MC Particle in Overlay file so assuming it is cosmic" << std::endl;            
            ...
            particle_container.SetpfpOpenAngle(pfp_open_angle); // This is in radians, so will want to change this to degrees at some point
