...
runsubrunlist=run1_run_subrun_list_mc.txt

# The name of the sam def which you made the run subrun event list from
originalsamdef=prodgenie_numi_uboone_overlay_fhc_mcc9_run1_v28_all_snapshot

# choose a name for your sam def, any name as long as it doesnt already exist
newsamdef=kmistry_test_12april2021
...
for s in ${runsubrunlist_arr[@]}; do
    ...
    echo "nuexsec_data_filtered_${newsamdef_arr[$counter]}.root /pnfs/uboone/scratch/users/kmistry/mcc9_filtered_files/run1/"
    mv nuexsec_data_filtered_${newsamdef_arr[$counter]}.root /pnfs/uboone/scratch/users/kmistry/mcc9_filtered_files/run1/
    echo
