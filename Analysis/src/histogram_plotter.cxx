...
void histogram_plotter::InitHistograms(){
    ...
    // Reco Vtx X
    h_reco_vtx_x_stack.resize(k_cuts_MAX);
    h_reco_vtx_y_stack.resize(k_cuts_MAX);
    h_reco_vtx_z_stack.resize(k_cuts_MAX);
    
    for (unsigned int i=0; i < cut_dirs.size();i++){

        h_reco_vtx_x_stack.at(i) = new THStack ( Form("h_reco_vtx_x_%s", cut_dirs.at(i).c_str()) ,"");
        h_reco_vtx_y_stack.at(i) = new THStack ( Form("h_reco_vtx_y_%s", cut_dirs.at(i).c_str()) ,"");
        h_reco_vtx_z_stack.at(i) = new THStack ( Form("h_reco_vtx_z_%s", cut_dirs.at(i).c_str()) ,"");
        
    }

}
// -----------------------------------------------------------------------------
// void histogram_plotter::SetStack(std::vector<TH1D*> hist, bool area_norm,  bool logy, const char* x_axis_name,
